...
/// @dev This is a general zap contract for LendFlareVaultZap,
///      everything will zip into ETH or CRV and send back to caller.
contract LendFlareVaultZap is Ownable, IZap {
  ...
  function zap(
    ...
    require(_toToken == WETH || _toToken == CRV, "LendFlareVaultZap: toToken unsupported");

    uint256[] memory _routes = routes[_fromToken][_toToken];
    require(_routes.length > 0, "LendFlareVaultZap: route unavailable");

    uint256 _amount = _amountIn;
    for (uint256 i = 0; i < _routes.length; i++) {
      _amount = _swap(_routes[i], _amount);
    }
    require(_amount >= _minOut, "LendFlareVaultZap: insufficient output");
    if (_toToken == WETH) {
      _unwrapIfNeeded(_amount);
      // solhint-disable-next-line avoid-low-level-calls
      (bool success, ) = msg.sender.call{ value: _amount }("");
      require(success, "LendFlareVaultZap: ETH transfer failed");
    } else {
  ...
  function _swap(uint256 _route, uint256 _amountIn) internal returns (uint256) {
    ...
    if (_poolType == 0) {
      ...
      revert("LendFlareVaultZap: invalid poolType");
    }
  }
